...
def ActPirate(pirate):
    ...
    if sigT!="":
        ...
        sigT_list.sort()
        for signal in sigT_list:
            if int(signal[0])=="1":
                island1.append(signal)
            elif signal[0]=="2":
                island2.append(signal)
            elif signal[0]=="3":
                island3.append(signal)
        
        if len(island1)==island_guards:
            if len(island2)==island_guards:
                if len(island3)==island_guards:
                    return central_pirates(pirate)
                else:
                    return moveTo(####island3 coords)
            else:
                return moveTo(####island2 coords)
        else:
            return moveTo(####island1 coords)
